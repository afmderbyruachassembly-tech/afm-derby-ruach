...
const ChurchLeadership: FC<ChurchLeadershipProps> = ({ slice }) => {
  return (
    ...
    >
      <Bounded>
        <div className="m-auto max-w-4xl text-4xl leading-[115%] font-black tracking-tighter text-balance capitalize lg:text-center xl:text-6xl">
          {slice.primary.title}
        </div>
        ...
        <div className="card_container mt-10 overflow-hidden">
          <div className="[--corner-radius:16px] md:grid md:grid-cols-12 md:gap-4">
            {slice.primary.card.map((item) => (
              <div
                key={item.name}
                className='bentocard border-afm-card-bg before:bg-afm-card-bg relative rounded-[var(--corner-radius)] border p-2 before:absolute before:inset-0 before:-z-10 before:m-[calc(var(--corner-radius)/2)] before:rounded-[calc(var(--corner-radius)/2)] before:content-[""] sm:col-span-6 lg:col-span-4 xl:col-span-3'
              >
                <PrismicNextImage
                  field={item.profile}
